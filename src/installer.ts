    ...
    "tool.execute.after": async (input: { tool: string; sessionID: string }, output: { title: string; metadata: any }): Promise<void> => {
      ...
      if (afterEnabled && toolCount >= CONFIG.TOOL_THRESHOLD) {
        ...
        if (now - lastLearningPrompt >= CONFIG.COOLDOWN_MS) {
          ...
          setTimeout(() => {
            if (lastSessionID) {
              client.tui.showToast({ body: { title: "🎓 VibeLearning", message: "Learning time!", variant: "info", duration: 3000 } }).catch(() => {});
...
const platforms: Platform[] = [
  {
    ...
    install: () => {
      ...
      console.log("  ✓ Plugin installed: ~/.config/opencode/plugin/vibe-learning.ts");
      ...
      console.log("  ✓ Dependencies added to package.json (better-sqlite3)");
      ...
      console.log("  ✓ Cache cleared (dependencies will reinstall on startup)");
